(function() {
    ...
    var E = {
        ...
        isRduo: function(s) {
            var reg = /^(0[0-9]{2,3}-)?([2-9][0-9]{6,7})+(-[0-9]{1,6})?$/;
            var rsa = /^(1[34578][0-9]{1})[0-9]{8}$/;
            if(reg.exec||rsa.exec)
            {
                return true;
            }else
                {
         ...
         * @param args
         */
        ajax_post: function( args ,is_async) {
        ...
        /**
         * 生条形码
         * * @param id     //需要显示条码的标签ID
         * @param barcode  //条码值
         * @param params   //其他自定义参数
         */
        generateBarcode: function( id, barcode, params ) {
            var settings = {
                output:'css',           //输出条形码方式
                btype:'code128',       //生码规则
                ...
                color: '#000000',     //条形码颜色
                barWidth: '2px',      //条形码宽度
                barHeight: '50px',  //条形码高度
                numSize: '30px',     //条形码下方数字的大小
                moduleSize: 5,
                addQuietZone: false,
                isNeedNumber: 1  //是否需要显示条形码下方的数字
            };
            if (params) {
                settings = this.concat(settings, params);
            }

            $("#"+id).barcode(barcode, settings.btype, settings);
            if( settings.isNeedNumber == 1 ){
                $("#"+id).children(":last").css('font-size',settings.numSize);
            } else {
         ...
         * @param params   //其他自定义参数
         */
        generateQrCode: function( id, txt, params ) {

            //中文字符处理
            var text = this.toUtf8(txt);

            var settings = {
                render: "canvas", //canvas（默认）
                width: 200, //宽度
                height:200, //高度
                text: text
            }
